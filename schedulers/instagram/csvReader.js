...
const CSV_PATH = path.join(__dirname, '../../instagram_schedule.csv');
...
function readCSV() {
    ...
    try {
        ...
        logger.error(`Error reading/parsing CSV for Instagram: ${err.message}`);
        return [];
    }
}
...
function updateCSV(records) {
    try {
        ...
        logger.error(`Error writing CSV after Instagram update: ${err.message}`);
        return false;
    }
}
